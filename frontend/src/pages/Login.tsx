...
type FromState = string | { pathname?: string } | null | undefined;

function resolveFromPath(stateFrom: FromState): string {
  if (!stateFrom) return '/';
  if (typeof stateFrom === 'string') return stateFrom;
  return stateFrom.pathname || '/';
}

function getRedirectPath(from: string, role: string): string {
